...
_CLASS_TOKEN_PATTERN = re.compile(r"^[A-Za-z_][A-Za-z0-9_-]{0,63}$")
_CLASS_STATE_WORDS = frozenset(
    {
        "active",
        "checked",
        "closed",
...
_GENERATED_CLASS_PATTERNS = (
    ...
    re.compile(r"\d{4,}"),
    re.compile(r"^(?:css|emotion|jsx|mantine|radix|sc)-[a-z0-9]{5,}$", re.IGNORECASE),
)
...
@dataclass(frozen=True)
class ClassLocator:
    """Hold one filtered class token as a locator hint, never witness evidence."""
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.tag, str) or not _TAG_PATTERN.fullmatch(self.tag):
            raise ValueError("class locator tag must be a lowercase HTML tag")
        if not is_admissible_class_token(self.class_name):
            raise ValueError("class locator requires one filtered class token")


@dataclass(frozen=True)
class XPathLocator:
    """Hold one filtered anchored or canonical absolute XPath hint."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.mode == "absolute":
            ...
            raise ValueError("anchored xpath is not an admitted canonical shape")
        _xpath_string_literals(self.expression)


BrowserLocator: TypeAlias = RoleLocator | CssLocator | ClassLocator | XPathLocator
...
def _locator_from_mapping(value: object) -> BrowserLocator:
    ...
    if strategy == "class":
        if set(value) != {"by", "tag", "class"}:
            raise ValueError("invalid class locator fields")
        return ClassLocator(  # type: ignore[arg-type]
            tag=value.get("tag"),
...
def is_admissible_class_token(value: object) -> bool:
    """Accept one bounded non-state, non-generated class locator token."""

    if not isinstance(value, str) or not _CLASS_TOKEN_PATTERN.fullmatch(value):
        return False
    words = {word for word in re.split(r"[-_]", value.casefold()) if word}
    if words & _CLASS_STATE_WORDS:
        return False
    return not any(pattern.search(value) for pattern in _GENERATED_CLASS_PATTERNS)


def locator_from_payload(value: object) -> BrowserLocator:
    """Decode one strict locator payload for optional external candidate providers."""

    return _locator_from_mapping(value)


def _locator_rank(locator: BrowserLocator) -> int:
    """Return the fixed CSS, role, class, anchored XPath, absolute XPath order."""
